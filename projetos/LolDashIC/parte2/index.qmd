...
2. Com o `output$Nome` selecionamos o output que quero trabalhar. A função `renderInfoBox` diz que quero renderizar uma caixa de informações. Em seguida, chamo a função `infoBox` para passar os parametros que conterá em cada caixa de informação que são o título, o valor, o ícone e a cor
